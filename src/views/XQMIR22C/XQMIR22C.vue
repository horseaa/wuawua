...
const queryDetailed = async (row:any) => {
  var cusFilter ='';
 // console.log(row['SAMPLE_ENTR_NO'])
  cusFilter =`SAMPLE_ENTR_NO = '`+row['SAMPLE_ENTR_NO']+`'`;//配数据源，用sql语句查询
  await erFormHelper.queryDataByDataSource('gridView2',  { filterConfigId: 'LayoutGroupFilter',customFilter: cusFilter});
}

//查询未组汽车信息
const queryDetailed2 = async () => {
  const layoutValue: any = erFormHelper.getControlValue(//这个方法获取的日期为null
      'LayoutGroupFilter2',
      'WORK_DATE_START,WORK_DATE_END,SUPPLIER_CODE,MAT_CODE,TRNP_MODE_CODE',
      // 'DateEdit,DateEdit,GridLookupEdit,GridLookupEdit,TextEdit'
  );
